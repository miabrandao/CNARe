...
#Inserindo as recomendações geradas pelo Corals...

cursor.execute("SELECT autor1, autor2, ((1*similaridade)+(100*proximidade_social))/(1+100) as value FROM recomendacao WHERE cooperacao=0 AND (((1*similaridade)+(100*proximidade_social))/(1+100)) > 2/3 ORDER BY (((1*similaridade)+(100*proximidade_social))/(1+100)) DESC")
...
for recomendacao in cursor.fetchall():
  ...
  numrec = cursor.execute("SELECT * FROM recomenda WHERE pesquisador1_id = {p1_id} AND pesquisador2_id = {p2_id} AND metodo_id = 3".format(p1_id = p1, p2_id = p2))
  if numrec == 0:
    #Inserindo a recomendacao no banco...
    cursor.execute("INSERT INTO recomenda (pesquisador1_id, pesquisador2_id, metodo_id, value) VALUES ({p1_id}, {p2_id}, 3, {val})".format(p1_id = p1, p2_id = p2, val = recomendacao[2]))
    db.commit()
