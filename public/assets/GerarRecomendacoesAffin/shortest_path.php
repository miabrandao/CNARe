...
class shortest_path 
{
	...
	public function define_shortest_path()
	{
		...
		$query = mysql_query('SELECT DISTINCT autor1, autor2, cooperacao FROM '.$this->configDB->tabelaRecomendacao.' ORDER BY autor1');
		$node_anterior1 = NULL;
		$node_anterior2 = NULL;
		...
		$query = mysql_query('SELECT DISTINCT autor1, autor2 FROM '.$this->configDB->tabelaRecomendacao.' WHERE cooperacao = 0 ORDER BY autor1');	
		...
		while($fetch = mysql_fetch_array($query)) 
		{
			...
			{
				$sucesso &= mysql_query('UPDATE '.$this->configDB->tabelaRecomendacao.' SET menor_caminho = '.$menor_cam.' WHERE autor1='.$fetch["autor1"].' AND autor2='.$fetch["autor2"]);
			}
			else
			{
				$sucesso &= mysql_query('UPDATE '.$this->configDB->tabelaRecomendacao.' SET menor_caminho = 0 WHERE autor1='.$fetch["autor1"].' AND autor2='.$fetch["autor2"]);
				$menor_cam = 0;
		...
		while(!empty($Q))
		{
			...
			foreach($nodeList[$u]['children'] as $v)
			{
				...
				if($nodeList[$v]['color'] == 0)
				{
					...
					//print_r($nodeList);
					$sql = mysql_query('SELECT cooperacao FROM '.$this->configDB->tabelaRecomendacao.' WHERE autor1='.$u.' AND autor2='.$v);
	...
	private function calcula_max()
	{
		...
		$query = mysql_query('SELECT MAX(menor_caminho) as max_menor_caminho FROM '.$this->configDB->tabelaRecomendacao);	
		
		$max = mysql_result($query, 0);

		echo "\nMAX: ".$max."\n\n";

		$query = mysql_query('SELECT autor1, autor2, menor_caminho FROM '.$this->configDB->tabelaRecomendacao.' WHERE menor_caminho IS NOT NULL');
		...
		while($fetch = mysql_fetch_array($query)) 
		{
			...
			if($fetch["menor_caminho"] != 0)
			{
				...
				$sucesso &= mysql_query('UPDATE '.$this->configDB->tabelaRecomendacao.' SET proximidade_social = '.$proc_social.' WHERE autor1='.$fetch["autor1"].' AND autor2='.$fetch["autor2"]);
			...
			{
				$sucesso &= mysql_query('UPDATE '.$this->configDB->tabelaRecomendacao.' SET proximidade_social = 0 WHERE autor1='.$fetch["autor1"].' AND autor2='.$fetch["autor2"]);
